function [] = Circular_Aperture_NaN_Fixed(fid, var)
...
%Bottom-Right
% MM=-(M_x)./2.*0:a:(M_x)./2.*a;
% NN=-(N_y)./2.*0:a:(N_y)./2.*a;

%Top-Left
% MM=-(M_x)./2.*a:a:(M_x)./2.*0;
% NN=-(N_y)./2.*a:a:(N_y)./2.*0;
...
%% Gap Calculation
g = - (5.043e-7).*(Z_s.^3) + 0.0005039.*(Z_s.^2) - 0.1721.*Z_s + 20.24; %cubic Polynomial Mode_1
