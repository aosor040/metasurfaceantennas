function [] = Single_Source_Theta_0_Theta_60_17GHz_Combined(fid, var)
...
%         Z_s(ii,jj) = imag(j.*(X + Mdl./2.*(real(psi_obj_1(ii,jj).*conj(psi_ref(ii,jj))) ...
%             + real(psi_obj_2(ii,jj).*conj(psi_ref(ii,jj))))));
            else xx>0
        r(ii,jj) = sqrt((xx).^2 + yy.^2); 
        psi_obj_2(ii,jj) = exp(-j.*k_t.*xx.*sin(theta_2)); 
        psi_ref(ii,jj) = exp(-j.*k_t.*n.*r(ii,jj)); 
